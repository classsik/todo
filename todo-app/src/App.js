...
function App() {
  ...
  const onCompleteTask = (listId, taskId, completed) => {
    ...
    setLists(newList);
    axios
      .patch("http://localhost:8000/tasks/" + taskId, { completed: completed })
      .catch((err) => {
        console.log(err);
      });
  };
  ...
  const onRemoveTask = (listId, taskId) => {
    if (window.confirm("Хочешь удалить?")) {
      ...
      setLists(newList);
      axios.delete("http://localhost:8000/tasks/" + taskId).catch((err) => {
        console.log(err);
      });
    }
  };
  ...
  const onEditTask = (listId, taskObj) => {
    ...
    setLists(newList);
    axios
      .patch("http://localhost:8000/tasks/" + taskObj.id, { text: newTaskText })
      .catch((err) => {
        console.log(err);
      });
  };
